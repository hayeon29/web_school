...
export async function getLoginResult(req, res) {
  ...
  try {
    ...
    if (!existing_user) {
      ...
      try {
        const correct_password = await checkPassword(
          password,
...
export async function withdrawUser(req, res) {
  ...
  try {
    ...
    if (!existing_user) {
      ...
      req.session.destroy();
      //토큰(DB) 삭제
      await Token.findOneAndDelete({ id: req.body.user_id })
        .then(() => {
          console.log("user's refresh token successfully deleted");
        })
        .catch((err) => {
          console.log(
            "some error occurred and refresh token data didn't be deleted. error name: ",
            err
          );
          return res.json({
      ...
      //글(DB) 삭제
      await AnonymousPost.deleteMany({ id: req.body.user_id })
        .then(() => {
          console.log("user's anonymous post data successfully deleted");
        })
        .catch((err) => {
          console.log(
            "some error occurred and anonymous post data didn't be deleted. error name: ",
            err
          );
          return res.json({
      ...
      await Notice.deleteMany({ id: req.body.user_id })
        .then(() => {
          console.log("user's notice data successfully deleted");
        })
        .catch((err) => {
          console.log(
            "some error occurred and notice data didn't be deleted. error name: ",
            err
          );
          return res.json({
      ...
      //유저(DB) 삭제
      await User.findOneAndDelete({ id: req.body.user_id })
        .then(() => {
          console.log("user ", req.body.user_id, "was successfully deleted.");
        })
        .catch((err) => {
          console.log(
            "some error occurred and user info data didn't be deleted. error name: ",
            err
          );
          return res.json({
