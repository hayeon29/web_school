...
const cal_descript = document.getElementsByClassName(
  "calendar_descript_container"
)[0];
...
function renderCalendar(today) {
  ...
  document.querySelector(
    ".year_month"
  ).innerHTML = `<span class = 'year'> ${cur_year}년 </span>&nbsp<span class = 'year'> ${
    cur_month + 1
  }월 </span>`;
  ...
  console.log(
    `start_day = ${start_day}, prev_date = ${prev_date}, prev_day = ${prev_day}, end_day = ${end_day}, next_date = ${next_date}, next_day =  ${next_day}`
  );

  for (let i = prev_date - prev_day; i <= prev_date; i++) {
  ...
  for (let i = 1; i <= (7 - next_day == 1 ? 0 : 7 - next_day - 1); i++) {
    calendar.insertAdjacentHTML(
      "beforeend",
  ...
  fetch("/getdate", {
    ...
    .then((data) => {
      ...
      for (let i = 0; i < data.calendars.length; i++) {
        ...
        str +=
          '    <span class = "calendar_descript_title">' +
          getFormatDate(data.calendars[i].date) +
          "</span>";
        str +=
          '    <span class = "calendar_descript_content">' +
          data.calendars[i].content +
          "</span>";
        str += "</li>";
      }
